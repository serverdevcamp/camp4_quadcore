...
#     rdd = retweet.select('retweeted_status').rdd.map(lambda value: json.loads(value[0])) \
#         .filter(lambda v: v['truncated'] == True).map(lambda v : v['extended_tweet']).collect()
#     for i in rdd:
#         print(i)
#         print('-----')
    result = []
...
#             .format("org.apache.spark.sql.cassandra") \
#             .options(table="tweet_dataset", keyspace="bts") \
#             .load().select('extended_tweet','extended_entities') \
#             .where(col('date') == date) \
#             .where(col('hour') == hour) \
#             .where(col('timestamp') >= current_time - SECONDS) \
#             .where(col('timestamp') <= current_time).where(col('truncated') == True).cache()
#         tweet.printSchema()
        tweet.count()
        # 리트윗 데이터 
        retweet = spark.read \
